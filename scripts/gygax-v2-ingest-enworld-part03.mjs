#!/usr/bin/env node
/*
 * gygax-v2-ingest-enworld-part03.mjs — ingest the complete Part III preservation
 * segment of the ENWorld "Q&A with Gary Gygax" thread (Col_Pladoh), 6 April to
 * 22 July 2003.
 *
 *   node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part03.mjs \
 *        <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]
 *
 * This EXTENDS an object the corpus already holds. Part III is a coverage
 ...
 * existing sequence_in_object; the stale Part III coverage row (which described
 * v1-era screenshots that never reached v2) is rewritten to what is now held.
 *
 * Established Part-III-specific facts, each verified before this ran:
 *
 *   - 154 Gary posts out of 351 printable-view posts; the rest are participants'.
 *   - No overlap with the corpus: no existing unit cites Part III, and none falls
 *     inside the 6 Apr - 22 Jul 2003 window. Part II ends 6 Apr 14:19; this opens
 *     6 Apr 18:29.
 ...
 *
 * 154 cards = 109 source-native crops + 45 stitched across printable pagination
 * (45 joins). The stitches are content-affecting reconstructions and must pass
...
if (!DB_PATH || !PKG || !EVID) {
  console.error('Usage: node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part03.mjs <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]');
  process.exit(2);
}
...
// ---- validate ---------------------------------------------------------------
const problems = [];
const EXPECT = { units: 154, contexts: 161, crops: 109, stitched: 45, portions: 199, posts: 351 };
const FAMILY = 'ENWorld Q&A', TITLE = 'Q&A with Gary Gygax', SEGMENT = 'Part III';
...
for (const u of units) {
  ...
  if (u.source_family !== FAMILY) problems.push(`${k}: source_family "${u.source_family}" — Part III must extend the existing family, not create one`);
  if (u.documentary_object !== TITLE) problems.push(`${k}: documentary_object "${u.documentary_object}" — Part III must extend the existing object, not create one`);
  ...
  if (!/^P03:post\d{4}$/.test(u.source_locator)) problems.push(`${k}: unrecognised locator ${u.source_locator}`);
...
console.log(`ENWorld Part III (complete) — package verified under schema ${SCHEMA.version}`);
...
if (!fam) { db.exec('ROLLBACK'); die(`source family "${FAMILY}" not found; Part III extends it and must not create a new one.`); }
const obj = g(`SELECT id FROM documentary_objects WHERE family_id=? AND title=?`, fam.id, TITLE);
if (!obj) { db.exec('ROLLBACK'); die(`documentary object "${TITLE}" not found; Part III is a coverage segment of it, not a new object.`); }
const existing = g(`SELECT count(*) c, coalesce(max(sequence_in_object),0) hi FROM testimony_units WHERE object_id=?`, obj.id);
const already = g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND (source_locator LIKE '%Part III%' OR source_locator LIKE '%enweggqa03%')`, obj.id).c;
if (already && !FORCE) { db.exec('ROLLBACK'); die(`the object already holds ${already} Part III unit(s); use --force on a clean DB.`); }
...
const DIR = 'evidence/enworld/part03/';
...
for (const u of units) {
  ...
  const locator = `enweggqa03.pdf PDF pages ${lo}-${hi}; printable-view position ${u.printable_view_position} (${SEGMENT})`;
  seq += 1;
  ...
  for (const p of a.source_portions) {
    ...
    insSrc.run(aid, `enweggqa03.pdf p.${p.source_page}, clip [${p.clip_pdf_points.join(', ')}] pt — ${what}`);
  }
...
// The Part III coverage row described v1-era screenshots that never reached v2:
// the object held ZERO Part III units before this run. Rewrite it to what is held.
db.prepare(`UPDATE coverage SET segment_kind='preservation_part', coverage_status='complete', known_loss=0,
  ...
  `Complete Part III printable-view preservation PDF (enweggqa03.pdf, 96 pages) ingested post by post: ${units.length} Gygax posts out of ${COV.printable_posts_total} printable-view posts, ${COV.first_locator} to ${COV.last_locator}, ${COV.first_date} to ${COV.last_date}. Complete AS PRESERVED for Gygax's own posts in this segment; the other ${COV.printable_posts_total - units.length} posts are participants' and are held only where Gygax quoted them back. Supersedes the v1 description of selected screenshots around ENWorld pages 103-110, none of which reached the v2 corpus.`,
  obj.id, SEGMENT);
...
insAnn.run(u1, 'segment_scope',
  `Part III is a COVERAGE SEGMENT of this thread, not a separate documentary object: it extends "${TITLE}" rather than creating one. ${units.length} Gygax posts were added, continuing sequence_in_object from ${existing.hi}. Verified before ingest that the corpus held no Part III unit and none dated inside ${first}..${last}, so nothing is double-ingested; Part II ends 2003-04-06T14:19 and this segment opens 2003-04-06T18:29.`);
...
insAnn.run(u1, 'attribution',
  `Gygax's words in this segment are frequently NON-CONTIGUOUS: a single post may carry several quote boxes with his replies between them. Discovery holds his own words only; each quoted prompt is a separate context row (${ctxRows.length} across ${ctxByUnit.size} units, up to six on one post), attributed to the handle the source names and left with NO speaker on the ${nUnattr} quote-backs the source does not attribute. Handles are source-scoped to "${HANDLE_SCOPE}"; an identical string in another forum stays a separate unresolved identity.`);
insAnn.run(u1, 'preparation_correction',
  'Pre-ingestion review rejected two earlier preparations of this segment. The first carried the wrong family and object names (which would have split this thread into a duplicate object), an undecoded HTML entity in a handle, printable-view navigation furniture inside context text, and a two-quote post whose second, unattributed quote had been attributed to the first quote\'s author. The second was correct except that one card (P03:post0164) stitched a page-44 portion containing NO ink: the post ends on page 43, so it was a reconstruction that reconstructed nothing. Corrected upstream to a single-portion crop; the other 153 card files stayed byte-identical, so their verification carried over.');
...
P('\nReconciliation report — ENWorld Part III (complete segment)');
P(`  documentary objects  : +${after.objects - before.objects} (0 expected — Part III EXTENDS "${TITLE}")`);
P(`  source families      : +${after.families - before.families} (0 expected — extends "${FAMILY}")`);
P(`  testimony units      : +${after.units - before.units} (expected 154; sequence ${existing.hi + 1}..${seq})`);
P(`  unit_context rows    : +${after.context - before.context} (expected 161 quoted prompts)`);
P(`  discovery_text rows  : +${after.discovery - before.discovery} (expected 315 = 154 replies + 161 prompts)`);
P(`  evidence assets      : +${after.assets - before.assets} (109 crop + 45 stitched)`);
P(`  provenance rows      : +${after.sources - before.sources} (expected 199 portions)`);
...
P(`  coverage segments    : +${after.coverage - before.coverage} (0 expected — the Part III row was REWRITTEN, not added)`);
...
ok('154 Gygax units, direct commentary, untranscribed',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND speaker_id=? AND evidence_relationship='direct' AND discourse_mode='commentary' AND transcript_status='untranscribed' AND transcript=''`, obj.id, existing.hi, gygax.id).c === 154);
...
ok('every new unit carries a minute-precision date inside the segment window',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND date_precision='minute' AND date_value BETWEEN ? AND ?`, obj.id, existing.hi, first, last).c === 154);
...
ok('161 quoted_question contexts, contiguous per unit',
   g(`SELECT count(*) c FROM unit_context WHERE ${inNew} AND context_type='quoted_question'`).c === 161 &&
...
ok('315 discovery rows split 154 replies / 161 prompts',
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%Gygax reply%'`).c === 154 &&
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%NOT Gygax-authored%'`).c === 161);
...
ok('no Gygax reply row carries forum furniture',
   replyRows.every((r) => !/Originally posted by/i.test(r.text)
     && !/(?:^|\n)[ \t]*Page \d+ of \d+[ \t]*(?:\n|$)/.test(r.text)
     && !/(?:^|\n)[ \t]*Quote:[ \t]*(?:\n|$)/.test(r.text)));
ok('109 crops + 45 stitched; 199 provenance portions',
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='crop'`).c === 109 &&
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='stitched'`).c === 45 &&
   g(`SELECT count(*) c FROM evidence_sources WHERE asset_id IN (SELECT id FROM evidence_assets WHERE ${inNew})`).c === 199);
...
ok('the Part III coverage row now describes what is held',
...
ok('no control character survived into any new row, and the sanitised glyphs are intact',
   newDisc.every((r) => !/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(r.text))
   && newDisc.filter((r) => r.text.includes(REPLACEMENT)).length === 2);

P('\nFTS behaviour (the §8 guarantee):');
ok('Gygax wording IS discoverable', g(`SELECT count(*) c FROM discovery_fts WHERE discovery_fts MATCH 'HackMaster'`).c >= 1);
ok('a quoted participant prompt is discoverable as CONTEXT, not as Gygax',
   g(`SELECT count(*) c FROM discovery_text d JOIN discovery_fts f ON f.rowid=d.id WHERE d.unit_id IN (${newUnits}) AND discovery_fts MATCH '"nonintelligent undead"' AND d.source_locator LIKE '%NOT Gygax-authored%'`).c >= 1);
ok('participant wording is NOT in Gygax transcript FTS', g(`SELECT count(*) c FROM units_fts WHERE units_fts MATCH '"nonintelligent undead"'`).c === 0);
...
console.log(`\n${verdict}`);
writeFileSync(join(dirname(DB_PATH), 'p3-stitched.json'), JSON.stringify(stitched, null, 1));
const logPath = DB_PATH.replace(/\.sqlite$/, '') + '.ingest-enworld-part03.log';
writeFileSync(logPath, ['Gygax corpus v2 — ENWorld "Q&A with Gary Gygax", Part III (complete preservation segment)',
