#!/usr/bin/env node
/*
 * gygax-v2-ingest-enworld-part04.mjs — ingest the complete Part IV preservation
 * segment of the ENWorld "Q&A with Gary Gygax" thread (Col_Pladoh), 22 July to
 * 9 December 2003.
 *
 *   node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part04.mjs \
 *        <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]
 *
 * This EXTENDS an object the corpus already holds. Part IV is a coverage
 ...
 * existing sequence_in_object; the Part IV coverage row (v1 status: "No complete
 * local source ingested") is rewritten to what is now held.
 *
 * Established Part-III-specific facts, each verified before this ran:
 *
 *   - 117 Gary posts out of 267 printable-view posts; the rest are participants'.
 *   - No overlap with the corpus: no existing unit cites Part IV, and none falls
 *     inside the 22 Jul - 9 Dec 2003 window. Part III ends 22 Jul 13:32; this opens
 *     the same afternoon at 16:42.
 ...
 *
 * 117 cards = 76 source-native crops + 41 stitched across printable pagination
 * (42 joins; one card carries two). The stitches are content-affecting reconstructions and must pass
...
if (!DB_PATH || !PKG || !EVID) {
  console.error('Usage: node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part04.mjs <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]');
  process.exit(2);
}
...
// ---- validate ---------------------------------------------------------------
const problems = [];
const EXPECT = { units: 117, contexts: 153, crops: 76, stitched: 41, portions: 159, posts: 267 };
const FAMILY = 'ENWorld Q&A', TITLE = 'Q&A with Gary Gygax', SEGMENT = 'Part IV';
...
for (const u of units) {
  ...
  if (u.source_family !== FAMILY) problems.push(`${k}: source_family "${u.source_family}" — Part IV must extend the existing family, not create one`);
  if (u.documentary_object !== TITLE) problems.push(`${k}: documentary_object "${u.documentary_object}" — Part IV must extend the existing object, not create one`);
  ...
  if (!/^P04:post\d{4}$/.test(u.source_locator)) problems.push(`${k}: unrecognised locator ${u.source_locator}`);
...
console.log(`ENWorld Part IV (complete) — package verified under schema ${SCHEMA.version}`);
...
if (!fam) { db.exec('ROLLBACK'); die(`source family "${FAMILY}" not found; Part IV extends it and must not create a new one.`); }
const obj = g(`SELECT id FROM documentary_objects WHERE family_id=? AND title=?`, fam.id, TITLE);
if (!obj) { db.exec('ROLLBACK'); die(`documentary object "${TITLE}" not found; Part IV is a coverage segment of it, not a new object.`); }
const existing = g(`SELECT count(*) c, coalesce(max(sequence_in_object),0) hi FROM testimony_units WHERE object_id=?`, obj.id);
const already = g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND (source_locator LIKE '%Part IV%' OR source_locator LIKE '%enweggqa04%')`, obj.id).c;
if (already && !FORCE) { db.exec('ROLLBACK'); die(`the object already holds ${already} Part IV unit(s); use --force on a clean DB.`); }
...
const DIR = 'evidence/enworld/part04/';
...
for (const u of units) {
  ...
  const locator = `enweggqa04.pdf PDF pages ${lo}-${hi}; printable-view position ${u.printable_view_position} (${SEGMENT})`;
  seq += 1;
  ...
  for (const p of a.source_portions) {
    ...
    insSrc.run(aid, `enweggqa04.pdf p.${p.source_page}, clip [${p.clip_pdf_points.join(', ')}] pt — ${what}`);
  }
...
// The Part IV coverage row described v1-era screenshots that never reached v2:
// the object held ZERO Part IV units before this run. Rewrite it to what is held.
db.prepare(`UPDATE coverage SET segment_kind='preservation_part', coverage_status='complete', known_loss=0,
  ...
  `Complete Part IV printable-view preservation PDF (enweggqa04.pdf, 96 pages) ingested post by post: ${units.length} Gygax posts out of ${COV.printable_posts_total} printable-view posts, ${COV.first_locator} to ${COV.last_locator}, ${COV.first_date} to ${COV.last_date}. Complete AS PRESERVED for Gygax's own posts in this segment; the other ${COV.printable_posts_total - units.length} posts are participants' and are held only where Gygax quoted them back. Supersedes the v1 description of selected screenshots around ENWorld pages 103-110, none of which reached the v2 corpus.`,
  obj.id, SEGMENT);
...
insAnn.run(u1, 'segment_scope',
  `Part IV is a COVERAGE SEGMENT of this thread, not a separate documentary object: it extends "${TITLE}" rather than creating one. ${units.length} Gygax posts were added, continuing sequence_in_object from ${existing.hi}. Verified before ingest that the corpus held no Part IV unit and none dated inside ${first}..${last}, so nothing is double-ingested; Part II ends 2003-04-06T14:19 and this segment opens 2003-04-06T18:29.`);
...
insAnn.run(u1, 'preparation_correction',
  'Pre-ingestion review rejected one earlier preparation of this segment. Three participant handles carried U+FFFD in place of a first letter that the PDF text layer had failed to extract: "�rthurQ", "�ndrew D. Gable" and "�hyron1144". Substituting U+FFFD is the correct repair for a failed glyph in prose, but a handle becomes a persons row, and khyron1144 also appeared INTACT elsewhere in the same package — so ingesting as supplied would have created two identities for one person inside a single segment. The glyphs are legible on the evidence cards (the text layer failed, not the rendering), so they were read there and corrected upstream to ArthurQ, Andrew D. Gable and khyron1144. All 117 card files and the source PDF stayed byte-identical, so the rest of the verification carried over.');
...
P('\nReconciliation report — ENWorld Part IV (complete segment)');
P(`  documentary objects  : +${after.objects - before.objects} (0 expected — Part IV EXTENDS "${TITLE}")`);
P(`  source families      : +${after.families - before.families} (0 expected — extends "${FAMILY}")`);
P(`  testimony units      : +${after.units - before.units} (expected 117; sequence ${existing.hi + 1}..${seq})`);
P(`  unit_context rows    : +${after.context - before.context} (expected 153 quoted prompts)`);
P(`  discovery_text rows  : +${after.discovery - before.discovery} (expected 270 = 117 replies + 153 prompts)`);
P(`  evidence assets      : +${after.assets - before.assets} (76 crop + 41 stitched)`);
P(`  provenance rows      : +${after.sources - before.sources} (expected 159 portions)`);
...
P(`  coverage segments    : +${after.coverage - before.coverage} (0 expected — the Part IV row was REWRITTEN, not added)`);
...
ok('117 Gygax units, direct commentary, untranscribed',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND speaker_id=? AND evidence_relationship='direct' AND discourse_mode='commentary' AND transcript_status='untranscribed' AND transcript=''`, obj.id, existing.hi, gygax.id).c === 117);
...
ok('every new unit carries a minute-precision date inside the segment window',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND date_precision='minute' AND date_value BETWEEN ? AND ?`, obj.id, existing.hi, first, last).c === 117);
...
ok('153 quoted_question contexts, contiguous per unit',
   g(`SELECT count(*) c FROM unit_context WHERE ${inNew} AND context_type='quoted_question'`).c === 153 &&
...
ok('270 discovery rows split 117 replies / 153 prompts',
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%Gygax reply%'`).c === 117 &&
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%NOT Gygax-authored%'`).c === 153);
...
ok('no Gygax reply row carries forum furniture',
   replyRows.every((r) => !/Originally posted by/i.test(r.text)
     && !/(?:^|\n)[ \t]*Page \d+ of \d+[ \t]*(?:\n|$)/.test(r.text)
     && !/(?:^|\n)[ \t]*Quote:[ \t]*(?:\n|$)/.test(r.text)));
ok('76 crops + 41 stitched; 159 provenance portions',
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='crop'`).c === 76 &&
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='stitched'`).c === 41 &&
   g(`SELECT count(*) c FROM evidence_sources WHERE asset_id IN (SELECT id FROM evidence_assets WHERE ${inNew})`).c === 159);
...
ok('the Part IV coverage row now describes what is held',
...
// Part IV needed no glyph substitution: the three failed handle glyphs were read off
...
console.log(`\n${verdict}`);
writeFileSync(join(dirname(DB_PATH), 'p4-stitched.json'), JSON.stringify(stitched, null, 1));
const logPath = DB_PATH.replace(/\.sqlite$/, '') + '.ingest-enworld-part04.log';
writeFileSync(logPath, ['Gygax corpus v2 — ENWorld "Q&A with Gary Gygax", Part IV (complete preservation segment)',
