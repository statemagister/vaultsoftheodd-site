#!/usr/bin/env node
/*
 * gygax-v2-ingest-enworld-part06.mjs — ingest the complete Part VI preservation
 * segment of the ENWorld "Q&A with Gary Gygax" thread (Col_Pladoh), 9 February
 * to 31 March 2004.
 *
 *   node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part06.mjs \
 *        <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]
 *
 * This EXTENDS an object the corpus already holds. Part VI is a coverage
 ...
 * existing sequence_in_object; the Part VI coverage row (v1 status: "No complete
 ...
 * against the PDF. Part VI declares NO such disagreement, and this ingester
 ...
 *
 *   - 105 Gary posts out of 217 printable-view posts; the rest are participants'.
 *   - No overlap with the corpus: no existing unit cites Part VI, and none falls
 *     inside the 9 Feb - 31 Mar 2004 window. Part V ends 9 Feb 22:08.
 ...
 *
 * 105 cards = 70 source-native crops + 35 stitched across printable pagination
 * (35 joins). The stitches are content-affecting reconstructions and must pass
...
if (!DB_PATH || !PKG || !EVID) {
  console.error('Usage: node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part06.mjs <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]');
  process.exit(2);
}
...
// ---- validate ---------------------------------------------------------------
const problems = [];
const EXPECT = { units: 105, contexts: 134, crops: 70, stitched: 35, portions: 140, posts: 217 };
const FAMILY = 'ENWorld Q&A', TITLE = 'Q&A with Gary Gygax', SEGMENT = 'Part VI';
...
for (const u of units) {
  ...
  if (u.source_family !== FAMILY) problems.push(`${k}: source_family "${u.source_family}" — Part VI must extend the existing family, not create one`);
  if (u.documentary_object !== TITLE) problems.push(`${k}: documentary_object "${u.documentary_object}" — Part VI must extend the existing object, not create one`);
  ...
  if (!/^P06:post\d{4}$/.test(u.source_locator)) problems.push(`${k}: unrecognised locator ${u.source_locator}`);
...
console.log(`ENWorld Part VI (complete) — package verified under schema ${SCHEMA.version}`);
...
if (!fam) { db.exec('ROLLBACK'); die(`source family "${FAMILY}" not found; Part VI extends it and must not create a new one.`); }
const obj = g(`SELECT id FROM documentary_objects WHERE family_id=? AND title=?`, fam.id, TITLE);
if (!obj) { db.exec('ROLLBACK'); die(`documentary object "${TITLE}" not found; Part VI is a coverage segment of it, not a new object.`); }
...
// EXACTLY: a bare LIKE '%Part VI%' also matches Part VII, VII and VIII, which made this
// guard fire against 157 Part VIIII units on the first run.
const already = g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND (source_locator LIKE ? OR source_locator LIKE '%enweggqa06%')`, obj.id, `%(${SEGMENT})%`).c;
if (already && !FORCE) { db.exec('ROLLBACK'); die(`the object already holds ${already} Part VI unit(s); use --force on a clean DB.`); }
...
const DIR = 'evidence/enworld/part06/';
...
for (const u of units) {
  ...
  const locator = `enweggqa06.pdf PDF pages ${lo}-${hi}; printable-view position ${u.printable_view_position} (${SEGMENT})`;
  seq += 1;
  ...
  for (const p of a.source_portions) {
    ...
    insSrc.run(aid, `enweggqa06.pdf p.${p.source_page}, clip [${p.clip_pdf_points.join(', ')}] pt — ${what}`);
  }
...
// The Part VI coverage row described v1-era screenshots that never reached v2:
// the object held ZERO Part VI units before this run. Rewrite it to what is held.
db.prepare(`UPDATE coverage SET segment_kind='preservation_part', coverage_status='complete', known_loss=0,
  ...
  `Complete Part VI printable-view preservation PDF (enweggqa06.pdf, 96 pages) ingested post by post: ${units.length} Gygax posts out of ${COV.printable_posts_total} printable-view posts, ${COV.first_locator} to ${COV.last_locator}, ${COV.first_date} to ${COV.last_date}. Complete AS PRESERVED for Gygax's own posts in this segment; the other ${COV.printable_posts_total - units.length} posts are participants' and are held only where Gygax quoted them back. Supersedes the v1 description of selected screenshots around ENWorld pages 103-110, none of which reached the v2 corpus.`,
  obj.id, SEGMENT);
...
insAnn.run(u1, 'segment_scope',
  `Part VI is a COVERAGE SEGMENT of this thread, not a separate documentary object: it extends "${TITLE}" rather than creating one. ${units.length} Gygax posts were added, continuing sequence_in_object from ${existing.hi}. Verified before ingest that the corpus held no Part VI unit and none dated inside ${first}..${last}, so nothing is double-ingested; Part II ends 2003-04-06T14:19 and this segment opens 2003-04-06T18:29.`);
...
if (labelMismatches.length) insAnn.run(u1, 'quote_label_mismatch',
  `Quote-backs in this segment whose vBulletin label disagrees with the preserved antecedent post header: ${labelMismatches.map((m) => `at ${m.ante} the header reads "${m.antecedent}" while the label reads "${m.label}"`).join('; ')}. The ANTECEDENT is canonical; the label is preserved in the provenance locator as linkage evidence only, per the rule established on Part V.`);
else insAnn.run(u1, 'quote_label_mismatch',
  'The Part V rule was applied and found nothing to apply it to: every quote-back in this segment whose vBulletin label is recorded agrees with the preserved antecedent post header, so no prompt needed the antecedent-over-label resolution. Checked, not assumed.');
insAnn.run(u1, 'segment_boundary_discontinuity',
  'The Part V/VI boundary runs BACKWARDS in displayed time. Part V\'s last Gygax post reads "Monday, 9th February, 2004, 10:08 PM"; the first three here read 07:42, 07:54 and 08:04 PM the same evening, though they follow it in thread order. Both were read from their own preservation PDFs and neither is a parse error. No content is duplicated: Gygax\'s 22:08 post appears only in the Part V PDF, these three only in the Part VI PDF, and no text in this package already existed in the corpus. Every other segment boundary in this object is monotonic, so the anomaly is confined to this join. A roughly three-hour difference in the capturing account\'s display timezone would reconcile it exactly — vBulletin renders timestamps in the viewer\'s zone — but that is a hypothesis, not established. CONSEQUENCE: ordering within a segment is sound, but stored minute-precision times may not share a frame ACROSS this boundary. Both the verbatim date_display and the normalised date_value are kept, so the question stays recoverable; sequence_in_object is an ordering key and asserts nothing historical. The date-window duplicate guard blocked this ingest on that discontinuity and was overridden only after duplication was excluded by content four ways.');
insAnn.run(u1, 'source_markup_defect',
  'vBulletin failed to parse the quote markup on four posts in this segment, so a literal "[/QUOTE]" is RENDERED as visible text in the preservation PDF (seven occurrences across the segment). The evidence cards preserve exactly what the source displays, defect included, because the card is the source-native rendering. The participant wording those broken tags enclose is classified as context and excluded from Gygax discovery, and the stray markup itself appears in neither text layer. The malformed tag is in fact useful: it marks where the quoted material ends and Gygax resumes.');
...
P('\nReconciliation report — ENWorld Part VI (complete segment)');
P(`  documentary objects  : +${after.objects - before.objects} (0 expected — Part VI EXTENDS "${TITLE}")`);
P(`  source families      : +${after.families - before.families} (0 expected — extends "${FAMILY}")`);
P(`  testimony units      : +${after.units - before.units} (expected 105; sequence ${existing.hi + 1}..${seq})`);
P(`  unit_context rows    : +${after.context - before.context} (expected 134 quoted prompts)`);
P(`  discovery_text rows  : +${after.discovery - before.discovery} (expected 239 = 105 replies + 134 prompts)`);
P(`  evidence assets      : +${after.assets - before.assets} (70 crop + 35 stitched)`);
P(`  provenance rows      : +${after.sources - before.sources} (expected 140 portions)`);
...
P(`  coverage segments    : +${after.coverage - before.coverage} (0 expected — the Part VI row was REWRITTEN, not added)`);
...
ok('105 Gygax units, direct commentary, untranscribed',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND speaker_id=? AND evidence_relationship='direct' AND discourse_mode='commentary' AND transcript_status='untranscribed' AND transcript=''`, obj.id, existing.hi, gygax.id).c === 105);
...
ok('every new unit carries a minute-precision date inside the segment window',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND date_precision='minute' AND date_value BETWEEN ? AND ?`, obj.id, existing.hi, first, last).c === 105);
...
ok('134 quoted_question contexts, contiguous per unit',
   g(`SELECT count(*) c FROM unit_context WHERE ${inNew} AND context_type='quoted_question'`).c === 134 &&
...
ok('239 discovery rows split 105 replies / 134 prompts',
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%Gygax reply%'`).c === 105 &&
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%NOT Gygax-authored%'`).c === 134);
...
ok('no Gygax reply row carries forum furniture',
   replyRows.every((r) => !/Originally posted by/i.test(r.text)
     && !/(?:^|\n)[ \t]*Page \d+ of \d+[ \t]*(?:\n|$)/.test(r.text)
     && !/(?:^|\n)[ \t]*Quote:[ \t]*(?:\n|$)/.test(r.text)));
ok('70 crops + 35 stitched; 140 provenance portions',
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='crop'`).c === 70 &&
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='stitched'`).c === 35 &&
   g(`SELECT count(*) c FROM evidence_sources WHERE asset_id IN (SELECT id FROM evidence_assets WHERE ${inNew})`).c === 140);
...
ok('the Part VI coverage row now describes what is held',
...
// Part VI needed no glyph substitution: the three failed handle glyphs were read off
// the evidence cards upstream, so NO U+FFFD should remain anywhere in this segment.
ok(`quote labels agree with their antecedents (${labelMismatches.length} disagreement(s), package declares 0)`,
...
console.log(`\n${verdict}`);
writeFileSync(join(dirname(DB_PATH), 'p6-stitched.json'), JSON.stringify(stitched, null, 1));
const logPath = DB_PATH.replace(/\.sqlite$/, '') + '.ingest-enworld-part06.log';
writeFileSync(logPath, ['Gygax corpus v2 — ENWorld "Q&A with Gary Gygax", Part VI (complete preservation segment)',
