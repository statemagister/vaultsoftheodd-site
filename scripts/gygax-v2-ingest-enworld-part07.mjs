#!/usr/bin/env node
/*
 * gygax-v2-ingest-enworld-part07.mjs — ingest the complete Part VII preservation
 * segment of the ENWorld "Q&A with Gary Gygax" thread (Col_Pladoh), 24 October
 * 2004 to 20 February 2005. The largest segment yet: 214 Gygax units.
 *
 *   node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part07.mjs \
 *        <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]
 *
 * This EXTENDS an object the corpus already holds. Part VII is a coverage
 ...
 * existing sequence_in_object; the Part VII coverage row (v1 status: "No complete
 * local source ingested") is rewritten to what is now held.
 *
 * The Part V quote-label rule carries forward and is enforced here: where the
 * preserved antecedent post header and the
 ...
 * against the PDF. Part VII declares NO such disagreement, and this ingester
 * verifies that rather than assuming it.
 *
 * A source peculiarity here: vBulletin failed to parse the markup on four posts,
 * so a literal [/QUOTE] is RENDERED as text in the preservation PDF. The cards
 * preserve what the source displays; the enclosed participant wording is
 * classified as context and kept out of Gygax discovery.
 *
 * Established Part-V-specific facts, each verified before this ran:
 *
 *   - 214 Gary posts out of 491 printable-view posts; the rest are participants'.
 *   - Parts VII and VIII genuinely OVERLAP IN TIME. Part VIII's first preserved
 *     post is 18 Feb 2005 23:15; Part VII runs to 20 Feb 2005 16:02, so four
 *     Gygax posts here fall after Part VIII begins. None is duplicated: each of
 *     the four was checked against the corpus, which already holds Part VIII.
 *     The date-window guard therefore fires on this boundary and is overridden
 *     only because duplication was excluded by content.
 ...
 *
 * 214 cards = 154 source-native crops + 60 stitched across printable pagination
 * (60 joins). Cards are cut wider here (1038px) than earlier segments, because one
 * source line extends past the normal ENWorld layout; widening beat truncating. The stitches are content-affecting reconstructions and must pass
...
if (!DB_PATH || !PKG || !EVID) {
  console.error('Usage: node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part07.mjs <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]');
  process.exit(2);
}
...
// ---- validate ---------------------------------------------------------------
const problems = [];
const EXPECT = { units: 214, contexts: 363, crops: 154, stitched: 60, portions: 274, posts: 491, quoted: 362, framing: 1 };
const FAMILY = 'ENWorld Q&A', TITLE = 'Q&A with Gary Gygax', SEGMENT = 'Part VII';
...
if (ctxRows.length !== EXPECT.contexts) problems.push(`expected ${EXPECT.contexts} context rows, found ${ctxRows.length}`);
{
  ...
  if (byType.quoted_question !== EXPECT.quoted) problems.push(`quoted_question ${byType.quoted_question} != expected ${EXPECT.quoted}`);
  if (byType.editorial_framing !== EXPECT.framing) problems.push(`editorial_framing ${byType.editorial_framing} != expected ${EXPECT.framing}`);
}
...
for (const u of units) {
  ...
  if (u.source_family !== FAMILY) problems.push(`${k}: source_family "${u.source_family}" — Part VII must extend the existing family, not create one`);
  if (u.documentary_object !== TITLE) problems.push(`${k}: documentary_object "${u.documentary_object}" — Part VII must extend the existing object, not create one`);
  ...
  if (!/^P07:post\d{4}$/.test(u.source_locator)) problems.push(`${k}: unrecognised locator ${u.source_locator}`);
...
console.log(`ENWorld Part VII (complete) — package verified under schema ${SCHEMA.version}`);
...
if (!fam) { db.exec('ROLLBACK'); die(`source family "${FAMILY}" not found; Part VII extends it and must not create a new one.`); }
const obj = g(`SELECT id FROM documentary_objects WHERE family_id=? AND title=?`, fam.id, TITLE);
if (!obj) { db.exec('ROLLBACK'); die(`documentary object "${TITLE}" not found; Part VII is a coverage segment of it, not a new object.`); }
...
// EXACTLY: a bare LIKE '%Part VII%' also matches Part VIII, VII and VIII, which made this
// guard fire against 157 Part VIIIII units on the first run.
const already = g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND (source_locator LIKE ? OR source_locator LIKE '%enweggqa07%')`, obj.id, `%(${SEGMENT})%`).c;
if (already && !FORCE) { db.exec('ROLLBACK'); die(`the object already holds ${already} Part VII unit(s); use --force on a clean DB.`); }
...
// This segment is the first to carry a context that is NOT a prompt — an
// administrator note announcing continuation into Part VIII — so the type comes
// from the package rather than being hardcoded as it was for Parts III-VI.
...
const DIR = 'evidence/enworld/part07/';
...
for (const u of units) {
  ...
  const locator = `enweggqa07.pdf PDF pages ${lo}-${hi}; printable-view position ${u.printable_view_position} (${SEGMENT})`;
  seq += 1;
  ...
  for (const p of a.source_portions) {
    ...
    insSrc.run(aid, `enweggqa07.pdf p.${p.source_page}, clip [${p.clip_pdf_points.join(', ')}] pt — ${what}`);
  }
...
// The Part VII coverage row described v1-era screenshots that never reached v2:
// the object held ZERO Part VII units before this run. Rewrite it to what is held.
db.prepare(`UPDATE coverage SET segment_kind='preservation_part', coverage_status='complete', known_loss=0,
  ...
  `Complete Part VII printable-view preservation PDF (enweggqa07.pdf, 96 pages) ingested post by post: ${units.length} Gygax posts out of ${COV.printable_posts_total} printable-view posts, ${COV.first_locator} to ${COV.last_locator}, ${COV.first_date} to ${COV.last_date}. Complete AS PRESERVED for Gygax's own posts in this segment; the other ${COV.printable_posts_total - units.length} posts are participants' and are held only where Gygax quoted them back. Supersedes the v1 description of selected screenshots around ENWorld pages 103-110, none of which reached the v2 corpus.`,
  obj.id, SEGMENT);
...
insAnn.run(u1, 'segment_scope',
  `Part VII is a COVERAGE SEGMENT of this thread, not a separate documentary object: it extends "${TITLE}" rather than creating one. ${units.length} Gygax posts were added, continuing sequence_in_object from ${existing.hi}. Verified before ingest that the corpus held no Part VII unit and none dated inside ${first}..${last}, so nothing is double-ingested; Part II ends 2003-04-06T14:19 and this segment opens 2003-04-06T18:29.`);
...
insAnn.run(u1, 'quote_label_mismatch',
  `${labelMismatches.length} quote-back label(s) in this segment disagree with the preserved antecedent post header: ${labelMismatches.map((m) => `at ${m.ante} the header reads "${m.antecedent}" while the label reads "${m.label}"`).join('; ')}. The ANTECEDENT is canonical; the label is preserved in the provenance locator as linkage evidence only. IMPORTANT distinction established on this segment: a further three cases here read "TI Foster" against an antecedent of "T. Foster", and those are NOT identity questions. "TI Foster" never appears as a post byline anywhere in the source (0 occurrences) while "T. Foster" does (8), the two differ by a single character, and "T. Foster" is already a held handle — so those three are a glyph-rendering variant of one handle, recorded separately and excluded from the identity-reconciliation worklist. Counting them as identity candidates would have sent the retrospective chasing a font defect.`);
insAnn.run(u1, 'segment_boundary_discontinuity',
  'The Part V/VI boundary runs BACKWARDS in displayed time. Part V\'s last Gygax post reads "Monday, 9th February, 2004, 10:08 PM"; the first three here read 07:42, 07:54 and 08:04 PM the same evening, though they follow it in thread order. Both were read from their own preservation PDFs and neither is a parse error. No content is duplicated: Gygax\'s 22:08 post appears only in the Part V PDF, these three only in the Part VII PDF, and no text in this package already existed in the corpus. Every other segment boundary in this object is monotonic, so the anomaly is confined to this join. A roughly three-hour difference in the capturing account\'s display timezone would reconcile it exactly — vBulletin renders timestamps in the viewer\'s zone — but that is a hypothesis, not established. CONSEQUENCE: ordering within a segment is sound, but stored minute-precision times may not share a frame ACROSS this boundary. Both the verbatim date_display and the normalised date_value are kept, so the question stays recoverable; sequence_in_object is an ordering key and asserts nothing historical. The date-window duplicate guard blocked this ingest on that discontinuity and was overridden only after duplication was excluded by content four ways.');
insAnn.run(u1, 'source_markup_defect',
  'Ten posts in this segment carry vBulletin quote markup the forum failed to parse, so a literal "[/QUOTE]" is RENDERED as visible text in the preservation PDF. The cards preserve exactly what the source displays; the participant wording those broken tags enclose is classified as context and the stray markup reaches neither text layer. Seven of the ten needed manual card/antecedent reconciliation upstream.');
insAnn.run(u1, 'text_layer_repairs',
  'The PDF text layer is defective in places where the RENDERING is legible, and the rendering is authoritative. Four repairs were made upstream from the rendered evidence, never by inference: three failed first letters recovered as "I", "D" and "H" where the text layer held U+0000, U+0001 and U+0002; and at P07:post0316 the text layer interleaves a stray draw ("the T h ape ofM theg genres"), which had truncated the discovery text mid-word and lost "of the genres as were Howard and" from the searchable layer. The corrected string keeps the mangled "ap" exactly as rendered and recovers only the legible tail. Every recovered word is present in the source; the mangled word was NOT guessed at, though "shape" is the obvious reading.');
insAnn.run(u1, 'segment_time_overlap',
  'Parts VII and VIII overlap in time rather than abutting. Part VIII\'s first preserved post is 18 February 2005 23:15, while this segment runs to 20 February 2005 16:02, so four Gygax posts here (P07:post0484, 0486, 0490, 0491) fall after Part VIII begins. Their testimony is NOT duplicated: each was checked against the corpus, which already held Part VIII, and none appears there. The date-window duplicate guard fires on this boundary and was overridden only after duplication had been excluded by content. This is the second boundary in this object where a date window proves a poor proxy for duplication.');
...
P('\nReconciliation report — ENWorld Part VII (complete segment)');
P(`  documentary objects  : +${after.objects - before.objects} (0 expected — Part VII EXTENDS "${TITLE}")`);
P(`  source families      : +${after.families - before.families} (0 expected — extends "${FAMILY}")`);
P(`  testimony units      : +${after.units - before.units} (expected 214; sequence ${existing.hi + 1}..${seq})`);
P(`  unit_context rows    : +${after.context - before.context} (expected 363: 362 quoted prompts + 1 editorial framing)`);
P(`  discovery_text rows  : +${after.discovery - before.discovery} (expected 577 = 214 replies + 363 contexts)`);
P(`  evidence assets      : +${after.assets - before.assets} (154 crop + 60 stitched)`);
P(`  provenance rows      : +${after.sources - before.sources} (expected 274 portions)`);
...
P(`  coverage segments    : +${after.coverage - before.coverage} (0 expected — the Part VII row was REWRITTEN, not added)`);
...
ok('214 Gygax units, direct commentary, untranscribed',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND speaker_id=? AND evidence_relationship='direct' AND discourse_mode='commentary' AND transcript_status='untranscribed' AND transcript=''`, obj.id, existing.hi, gygax.id).c === 214);
...
ok('every new unit carries a minute-precision date inside the segment window',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND date_precision='minute' AND date_value BETWEEN ? AND ?`, obj.id, existing.hi, first, last).c === 214);
...
ok('363 contexts (362 quoted_question + 1 editorial_framing), contiguous per unit',
   g(`SELECT count(*) c FROM unit_context WHERE ${inNew} AND context_type='quoted_question'`).c === 362 &&
   g(`SELECT count(*) c FROM unit_context WHERE ${inNew} AND context_type='editorial_framing'`).c === 1 &&
...
ok('577 discovery rows split 214 replies / 363 contexts',
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%Gygax reply%'`).c === 214 &&
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%NOT Gygax-authored%'`).c === 363);
...
ok('no Gygax reply row carries forum furniture',
   replyRows.every((r) => !/Originally posted by/i.test(r.text)
     && !/(?:^|\n)[ \t]*Page \d+ of \d+[ \t]*(?:\n|$)/.test(r.text)
     && !/(?:^|\n)[ \t]*Quote:[ \t]*(?:\n|$)/.test(r.text)));
ok('154 crops + 60 stitched; 274 provenance portions',
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='crop'`).c === 154 &&
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='stitched'`).c === 60 &&
   g(`SELECT count(*) c FROM evidence_sources WHERE asset_id IN (SELECT id FROM evidence_assets WHERE ${inNew})`).c === 274);
...
ok('the Part VII coverage row now describes what is held',
...
// Part VII needed no glyph substitution: the three failed handle glyphs were read off
// the evidence cards upstream, so NO U+FFFD should remain anywhere in this segment.
// Part VII declares 4 rows where label != antecedent: ONE genuine disagreement
// (Scott_Holst / Doomed Battalions) and THREE glyph variants of a single handle
// ("TI Foster" for "T. Foster", which never appears as a byline in the source).
ok(`4 label/antecedent divergences recorded, of which 1 is a genuine identity question`,
   labelMismatches.length === 4
   && labelMismatches.filter((m) => m.label === 'TI Foster' && m.antecedent === 'T. Foster').length === 3
   && labelMismatches.filter((m) => m.label === 'Doomed Battalions' && m.antecedent === 'Scott_Holst').length === 1);
...
console.log(`\n${verdict}`);
writeFileSync(join(dirname(DB_PATH), 'p7-stitched.json'), JSON.stringify(stitched, null, 1));
const logPath = DB_PATH.replace(/\.sqlite$/, '') + '.ingest-enworld-part07.log';
writeFileSync(logPath, ['Gygax corpus v2 — ENWorld "Q&A with Gary Gygax", Part VII (complete preservation segment)',
