#!/usr/bin/env node
/*
 * gygax-v2-ingest-enworld-part13a.mjs — ingest operational batch A of the Part XIII
 * preservation segment of the ENWorld "Q&A with Gary Gygax" thread (Col_Pladoh),
 * 10 April to 22 June 2007, P13:post0001 to post0540.
 *
 *   node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part13a.mjs \
 ...
 *     posts of the segment's 1606 printable-view posts; B and C resume after
 *     P13:post0540.
 *   - The batch is an UPLOAD boundary, never a historical one. Every unit records
 *     preservation_segment "Part XIII" and operational_batch "Part XIII-A", which
 ...
 *     the antecedent is canonical and the label is kept only as linkage evidence.
 *     Five such rows here, and they are two distinct phenomena: three are text-layer
 *     glyph loss where the damaged label is a truncation of the antecedent handle
 *     itself, and two are genuine attribution discrepancies (Nahat Anoj / Jonathan
 *     Moyer). Only the latter belong in the identity-reconciliation worklist.
 *
 * 224 cards = 174 source-native crops + 50 stitched across printable pagination
 * (50 joins), cut at 1038px as in Part VII. The stitches are content-affecting
...
if (!DB_PATH || !PKG || !EVID) {
  console.error('Usage: node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part13a.mjs <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]');
  process.exit(2);
}
...
// ---- validate ---------------------------------------------------------------
const problems = [];
const EXPECT = { units: 224, contexts: 199, crops: 174, stitched: 50, portions: 274, posts: 1606, quoted: 199, framing: 0 };
const FAMILY = 'ENWorld Q&A', TITLE = 'Q&A with Gary Gygax', SEGMENT = 'Part XIII', BATCH = 'Part XIII-A';
...
const DIR = 'evidence/enworld/part13a/';
...
db.prepare(`UPDATE coverage SET segment_kind='preservation_part', coverage_status='partial', known_loss=0,
  ...
  `Part XIII is being ingested in three operational batches from the 438-page printable-view preservation PDF (enweggqa13.pdf). Batch ${BATCH} is held: ${units.length} Gygax posts, ${COV.first_locator} to ${COV.last_locator}, ${COV.first_date} to ${COV.last_date}. The segment carries ${COV.printable_posts_total} printable-view posts in total and batches B and C are NOT yet held, so coverage is PARTIAL. The batch label is an upload boundary only: every unit records preservation segment "${SEGMENT}", and no batch appears as a segment of its own.`,
  obj.id, SEGMENT);
...
insAnn.run(u1, 'quote_label_divergence',
  `Five quote-back labels in this batch disagree with the preserved antecedent post header, and they are two different phenomena. THREE are text-layer glyph loss, where the damaged label is a truncation of the antecedent handle itself: "nterthorn" for Winterthorn, "canid" for Mycanid, "a dim" for Naidim. Those are rendering damage rather than identity questions, and are excluded from the identity-reconciliation worklist for the same reason the Part VII "TI Foster" cases were. TWO are genuine attribution discrepancies, where the antecedent header reads "Nahat Anoj" and the label reads "Jonathan Moyer". Worth recording for the eventual reconciliation: the handle is the given name REVERSED — "NahatAnoj" backwards reads "jonAtahaN" — so this is a pseudonymous handle set against a real name rather than an account rename. That makes it a strong candidate, and it also means resolving it touches real-name enrichment, which is deliberately deferred. Nothing is merged here: the antecedent stays canonical and the label is kept as linkage evidence in the provenance locator.`);
...
P(`  testimony units      : +${after.units - before.units} (expected 224; sequence ${existing.hi + 1}..${seq})`);
P(`  unit_context rows    : +${after.context - before.context} (expected 199 quoted prompts)`);
P(`  discovery_text rows  : +${after.discovery - before.discovery} (expected 423 = 224 replies + 199 contexts)`);
P(`  evidence assets      : +${after.assets - before.assets} (174 crop + 50 stitched)`);
...
ok('224 Gygax units, direct commentary, untranscribed',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND speaker_id=? AND evidence_relationship='direct' AND discourse_mode='commentary' AND transcript_status='untranscribed' AND transcript=''`, obj.id, existing.hi, gygax.id).c === 224);
...
ok('every new unit carries a minute-precision date inside the segment window',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND date_precision='minute' AND date_value BETWEEN ? AND ?`, obj.id, existing.hi, first, last).c === 224);
...
ok('199 quoted_question contexts, contiguous per unit',
   g(`SELECT count(*) c FROM unit_context WHERE ${inNew} AND context_type='quoted_question'`).c === 199 &&
...
ok('423 discovery rows split 224 replies / 199 contexts',
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%Gygax reply%'`).c === 224 &&
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%NOT Gygax-authored%'`).c === 199);
...
ok('no Gygax reply row carries forum furniture',
   replyRows.every((r) => !/Originally posted by/i.test(r.text)
     && !/(?:^|\n)[ \t]*Page \d+ of \d+[ \t]*(?:\n|$)/.test(r.text)
     && !/(?:^|\n)[ \t]*Quote:[ \t]*(?:\n|$)/.test(r.text)));
ok('174 crops + 50 stitched; 274 provenance portions',
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='crop'`).c === 174 &&
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='stitched'`).c === 50 &&
...
ok('the Part XIII coverage row records PARTIAL coverage and the range held',
   g(`SELECT count(*) c FROM coverage WHERE object_id=? AND segment_label=? AND coverage_status='partial' AND locator_from=? AND locator_to=? AND detail LIKE '%${units.length} Gygax posts%'`, obj.id, SEGMENT, COV.first_locator, COV.last_locator).c === 1);
...
console.log(`\n${verdict}`);
writeFileSync(join(dirname(DB_PATH), 'p13a-stitched.json'), JSON.stringify(stitched, null, 1));
const logPath = DB_PATH.replace(/\.sqlite$/, '') + '.ingest-enworld-part13a.log';
