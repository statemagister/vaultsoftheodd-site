#!/usr/bin/env node
/*
 * gygax-v2-ingest-enworld-part13b.mjs — ingest operational batch B of the Part XIII
 * preservation segment of the ENWorld "Q&A with Gary Gygax" thread (Col_Pladoh),
 * 25 June to 16 October 2007, P13:post0543 to post1080. The batch's source-position
 * scope opens at post0541, but 0541-0542 are participant posts, so the first Gygax
 * unit is post0543.
 *
 *   node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part13b.mjs \
 ...
 *
 * BATCHED SEGMENT — the first in this corpus:
 *
 *   - Part XIII is prepared in three operational batches. XIII-A holds 224 Gygax
 *     posts and is already ingested; this batch adds 235 more of the segment's
 *     1606 printable-view posts. C resumes after P13:post1080 and closes it.
 *   - The batch is an UPLOAD boundary, never a historical one. Every unit records
 *     preservation_segment "Part XIII" and operational_batch "Part XIII-B", which
 *     this ingester checks, so no phantom segment can be created.
 *   - Coverage is therefore recorded PARTIAL, naming only the locator range held.
 *     It becomes complete when C lands, not before.
 *   - The duplicate guard is scoped to this batch's LOCATOR RANGE rather than to
 *     the segment: B and C will legitimately add units to a segment that already
 *     has some, and a segment-scoped test would refuse them.
 *
 ...
 *     the antecedent is canonical. This batch declares ZERO genuine disagreements
 *     and three glyph-loss fragments (one Xyxox, two Geoffrey), and the ingester
 *     checks the observed set partitions exactly into the declared sets.
 *   - The batch also restores 18 quote labels, one discovery string, one context
 *     opening and two antecedent timestamps from the RENDERED card where the text
 *     layer was damaged. The text layer of this PDF substitutes "5" for "c" and
 *     "k" for "x" in places; nine of the restored handles are independently
 *     corroborated by already-held corpus identities, and the rest by byline
 *     frequency in the source itself.
 *
 * 235 cards = 179 source-native crops + 56 stitched across printable pagination
 * (56 joins), cut at 1038px as in Part VII. The stitches are content-affecting
 * reconstructions and must pass the eyes-on acceptance gate before any build.
 */
...
if (!DB_PATH || !PKG || !EVID) {
  console.error('Usage: node --experimental-sqlite scripts/gygax-v2-ingest-enworld-part13b.mjs <v2.sqlite> <package-dir> <evidence-staging-dir> [--force]');
  process.exit(2);
}
...
// ---- validate ---------------------------------------------------------------
const problems = [];
const EXPECT = { units: 235, contexts: 225, crops: 179, stitched: 56, portions: 291, posts: 1606, quoted: 225, framing: 0 };
const FAMILY = 'ENWorld Q&A', TITLE = 'Q&A with Gary Gygax', SEGMENT = 'Part XIII', BATCH = 'Part XIII-B';
...
const DIR = 'evidence/enworld/part13b/';
...
// The Part XIII coverage row described v1-era screenshots that never reached v2:
// the object held ZERO Part XIII units before this run. Rewrite it to what is held.
// Coverage names the CUMULATIVE range across batches ingested so far, not just this
// one: after B the segment holds A+B, and rewriting it with only B's range would
// silently drop A from the record of what is held.
...
db.prepare(`UPDATE coverage SET segment_kind='preservation_part', coverage_status='partial', known_loss=0,
   number_from=NULL, number_to=NULL, locator_from=?, locator_to=?, detail=? WHERE object_id=? AND segment_label=?`).run(
  heldLoc.lo, heldLoc.hi,
  `Part XIII is being ingested in three operational batches from the 438-page printable-view preservation PDF (enweggqa13.pdf). Batches through ${BATCH} are held: ${heldLoc.n} Gygax posts in total, ${heldLoc.lo} to ${heldLoc.hi}. The segment carries ${COV.printable_posts_total} printable-view posts and batch C is NOT yet held, so coverage remains PARTIAL. The batch label is an upload boundary only: every unit records preservation segment "${SEGMENT}", and no batch appears as a segment of its own.`,
  obj.id, SEGMENT);
...
insAnn.run(u1, 'operational_batching',
  `Part XIII is large enough to be prepared in three operational batches; this is ${BATCH}, covering ${COV.first_locator} to ${COV.last_locator}. The batch is an UPLOAD boundary and not a historical one: every unit records preservation segment "${SEGMENT}", the batch label never becomes a segment of its own, and the Part XIII coverage row is marked PARTIAL naming only the range actually held. The duplicate guard is scoped to this batch's locator range rather than to the segment, because batches B and C will legitimately add units to a segment that already has some — a segment-scoped test would refuse them.`);
insAnn.run(u1, 'quote_label_divergence',
  `Three quote-back labels in this batch disagree with the preserved antecedent post header, and all three are text-layer glyph loss rather than identity questions: the rendered label breaks the handle with stray spacing ("X y x" for Xyxox, "G eoffrey" twice for Geoffrey). The batch declares ZERO genuine attribution disagreements, and this ingester verified that the observed set partitions exactly into the declared sets rather than assuming it. The antecedent remains canonical in each case; the damaged label is kept in the provenance locator as linkage evidence only.`);
insAnn.run(u1, 'text_layer_restorations',
  `This batch's PDF text layer is systematically damaged in a way the rendering is not: it substitutes "5" for "c" and "k" for "x" in places, and drops leading letters. Restorations were therefore taken from the RENDERED card, never inferred: 18 quote labels, one discovery string (the book title "Anubis Murders", whose initial A the text layer loses), one context opening, and two antecedent AM timestamps. The restorations are independently corroborated rather than trusted: nine land on handles the corpus already holds while the damaged form exists nowhere, and the remainder are confirmed by byline frequency in the source itself — "Marshal Lucky" appears 20 times against 2 damaged, "Valiant" 6 against 1, "haakon1" 135. The one case running counter to the "5"-for-"c" pattern, JMac5892 against a text layer reading JMacc892, was settled by the byline: JMac5892 appears twice as a post header and never as a label, and the rendered card confirms it.`);
...
P(`  testimony units      : +${after.units - before.units} (expected 235; sequence ${existing.hi + 1}..${seq})`);
P(`  unit_context rows    : +${after.context - before.context} (expected 225 quoted prompts)`);
P(`  discovery_text rows  : +${after.discovery - before.discovery} (expected 460 = 235 replies + 225 contexts)`);
P(`  evidence assets      : +${after.assets - before.assets} (179 crop + 56 stitched)`);
P(`  provenance rows      : +${after.sources - before.sources} (expected 291 portions)`);
...
ok('235 Gygax units, direct commentary, untranscribed',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND speaker_id=? AND evidence_relationship='direct' AND discourse_mode='commentary' AND transcript_status='untranscribed' AND transcript=''`, obj.id, existing.hi, gygax.id).c === 235);
...
ok('every new unit carries a minute-precision date inside the segment window',
   g(`SELECT count(*) c FROM testimony_units WHERE object_id=? AND sequence_in_object>? AND date_precision='minute' AND date_value BETWEEN ? AND ?`, obj.id, existing.hi, first, last).c === 235);
...
ok('225 quoted_question contexts, contiguous per unit',
   g(`SELECT count(*) c FROM unit_context WHERE ${inNew} AND context_type='quoted_question'`).c === 225 &&
...
ok('460 discovery rows split 235 replies / 225 contexts',
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%Gygax reply%'`).c === 235 &&
   g(`SELECT count(*) c FROM discovery_text WHERE unit_id IN (${newUnits}) AND source_locator LIKE '%NOT Gygax-authored%'`).c === 225);
...
ok('no Gygax reply row carries forum furniture',
   replyRows.every((r) => !/Originally posted by/i.test(r.text)
     && !/(?:^|\n)[ \t]*Page \d+ of \d+[ \t]*(?:\n|$)/.test(r.text)
     && !/(?:^|\n)[ \t]*Quote:[ \t]*(?:\n|$)/.test(r.text)));
ok('179 crops + 56 stitched; 291 provenance portions',
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='crop'`).c === 179 &&
   g(`SELECT count(*) c FROM evidence_assets WHERE ${inNew} AND asset_type='stitched'`).c === 56 &&
   g(`SELECT count(*) c FROM evidence_sources WHERE asset_id IN (SELECT id FROM evidence_assets WHERE ${inNew})`).c === 291);
...
ok(`the Part XIII coverage row records PARTIAL coverage over the cumulative range held`,
   g(`SELECT count(*) c FROM coverage WHERE object_id=? AND segment_label=? AND coverage_status='partial' AND locator_from='P13:post0001' AND locator_to=?`, obj.id, SEGMENT, COV.last_locator).c === 1);
...
console.log(`\n${verdict}`);
writeFileSync(join(dirname(DB_PATH), 'p13b-stitched.json'), JSON.stringify(stitched, null, 1));
const logPath = DB_PATH.replace(/\.sqlite$/, '') + '.ingest-enworld-part13b.log';
